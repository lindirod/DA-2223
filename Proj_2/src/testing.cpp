...
int main() {
    ...
    std::cout << "Select which graph to use:" << std::endl;
    ...
    if (choice1 == 3) {
        std::cout << "Enter k for k-clusters: ";
        std::cin >> k;
    }
    ...
    switch (choice) {
        case 1:
            ...
            if (choice1 == 2)
                solveTSPTourism(filename);
            else if (choice1 == 3)
                solveHTourism(filename, k);
            break;
        case 2:
            filename = "shipping.csv";
            if (choice1 == 2)
                solveTSPShipping(filename);
            else if (choice1 == 3)
                solveHShipping(filename, k);
            break;
        case 3:
            filename = "stadiums.csv";
            if (choice1 == 2)
                solveTSPStadiums(filename);
            else if (choice1 == 3)
                solveHStadiums(filename, k);
            break;
        case 4:
            filename = "edges.csv";
            filename2 = "nodes.csv";
            if (choice1 == 2)
                solveTSPreal(filename, filename2);
            break;
        case 5:
