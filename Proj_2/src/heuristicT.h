     ...
     * 
     */
    int findNearestNeighborHT(int currentNode, const std::vector<bool>& visited, const std::vector<std::vector<double>>& distances) {
     ...
     *
     */
    std::vector<int> nearestNeighborApproximationT(const std::vector<int>& cluster, const std::vector<std::vector<double>>& distances) {
        ...
        for (int i = 0; i < numNodes - 1; ++i) {
            int next = findNearestNeighborHT(current, visited, distances);
            visited[next] = true;
            tour.push_back(next);
            current = next;
        }
    ...
    std::vector<std::vector<int>> performHierarchicalClusteringT(const std::vector<TourismData>& data, int numClusters) {
     ...
     *
     */
    std::vector<int> performDivideAndConquerT(const std::vector<std::vector<int>>& clusters, const std::vector<std::vector<double>>& distances) {
        std::vector<int> tour;
        
        for (const std::vector<int>& cluster : clusters) {
            std::vector<int> subTour = nearestNeighborApproximationT(cluster, distances);
    ...
    /**
     * @brief Solve the Tourism Traveling Salesman Problem (TSP) using hierarchical clustering, divide and conquer, and nearest neighbor 2-approximation.
     *
     * This function solves the Traveling Salesman Problem (TSP) using hierarchical clustering, divide and conquer,
     * and nearest neighbor 2-approximation. It loads the tourism data from the specified file, performs the necessary steps
    ...
    void solveHTourism(const std::string& filename, int numClusters) {
        ...
        std::vector<std::vector<int>> clusters = performHierarchicalClusteringT(tourismData, numClusters);

        // Perform divide and conquer for each cluster and get the sub-tour for each cluster
        std::vector<int> tour = performDivideAndConquerT(clusters, distances);
