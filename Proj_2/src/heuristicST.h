    ...
    #ifndef HEURISTICST_H
    #define HEURISTICST_H
     ...
     * 
     */
    int findNearestNeighborHST(int currentNode, const std::vector<bool>& visited, const std::vector<std::vector<double>>& distances) {
     ...
     *
     */
    std::vector<int> nearestNeighborApproximationST(const std::vector<int>& cluster, const std::vector<std::vector<double>>& distances) {
        ...
        for (int i = 0; i < numNodes - 1; ++i) {
            int next = findNearestNeighborHST(current, visited, distances);
            visited[next] = true;
            tour.push_back(next);
            current = next;
        }
    ...
    std::vector<std::vector<int>> performHierarchicalClusteringST(const std::vector<StadiumData>& data, int numClusters) {
     ...
     *
     */
    std::vector<int> performDivideAndConquerST(const std::vector<std::vector<int>>& clusters, const std::vector<std::vector<double>>& distances) {
        std::vector<int> tour;
        
        for (const std::vector<int>& cluster : clusters) {
            std::vector<int> subTour = nearestNeighborApproximationST(cluster, distances);
    ...
    /**
     * @brief Solve TSP using hierarchical clustering, divide and conquer, and nearest neighbor 2-approximation for the "stadiums.csv" file.
     *
     * This function solves the Traveling Salesman Problem (TSP) using hierarchical clustering, divide and conquer,
     * and nearest neighbor 2-approximation. It loads the stadium data from the specified file, performs the necessary steps
     ...
     *
     */
    void solveHStadiums(const std::string& filename, int numClusters) {
        std::vector<StadiumData> stadiumData = loadStadiumsGraph(filename);
        std::chrono::steady_clock::time_point startTime = std::chrono::steady_clock::now();

        int numNodes = stadiumData.size();
        ...
        std::vector<std::vector<int>> clusters = performHierarchicalClusteringST(stadiumData, numClusters);

        // Perform divide and conquer for each cluster and get the sub-tour for each cluster
        std::vector<int> tour = performDivideAndConquerST(clusters, distances);
